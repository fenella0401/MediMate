                    ...
                    if data['history'] == []:
                        ...
                        if data['user'] == record_data['user_question']:
                            ...
                            if '否' not in if_ask_response or '?' in response or '？' in response:
                                ...
                                with open('./user_records_1/'+str(index)+'.json', 'r', encoding='utf-8') as f:
                                    record_data = json.load(f)
                                record_data[model_name] = {'history': history, 'status': '4', 'category': '', 'starttime': data['start_time']}
                                with open('./user_records_1/'+str(index)+'.json', 'w', encoding='utf-8') as f:
                                    json.dump(record_data, f, indent=4, ensure_ascii=False)
                                return result
                            else:
                                ...
                                with open('./user_records_1/'+str(index)+'.json', 'r', encoding='utf-8') as f:
                                    record_data = json.load(f)
                                record_data[model_name] = {'history': history, 'status': '3', 'category': '', 'starttime': data['start_time'], 'endtime': time.time()}
                                with open('./user_records_1/'+str(index)+'.json', 'w', encoding='utf-8') as f:
                                    json.dump(record_data, f, indent=4, ensure_ascii=False)
                                return result
                        else:
