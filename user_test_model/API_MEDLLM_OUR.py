                ...
                if data['status'] != '4':
                    ...
                    if data['history'] == []:
                        ...
                        if '是' in system_response.choices[0].message.content:       
                            ...
                            if '否' not in if_ask_response:
                                result = {'user_id': index, 'response': response, 'history': history, 'status': '2', 'category': ''}
                                record_data = {'user_id': index, 'user_question': data['user'], model_name: {'history': history, 'status': '2', 'category': '', 'starttime': data['start_time']}}
                                with open('./user_records_1/'+str(index)+'.json', 'w', encoding='utf-8') as f:
                                    json.dump(record_data, f, indent=4, ensure_ascii=False)
                                return result
                            else:
                                result = {'user_id': index, 'response': response, 'history': [], 'status': '3', 'category': ''}
                                record_data = {'user_id': index, 'user_question': data['user'], model_name: {'history': history, 'status': '3', 'category': '', 'starttime': data['start_time'], 'endtime': time.time()}}
                                with open('./user_records_1/'+str(index)+'.json', 'w', encoding='utf-8') as f:
                                    json.dump(record_data, f, indent=4, ensure_ascii=False)
                                return result
                    ...
                    # 非第一次请求
                        ...
                        if '否' not in if_ask_response:
                            ...
                            record_data[model_name]['status'] = '2'
                            with open('./user_records_1/'+str(index)+'.json', 'w', encoding='utf-8') as f:
                                json.dump(record_data, f, indent=4, ensure_ascii=False)
                            return result
                        else:
                ...
                # 体验第二个系统 status=4 前一系统已体验结束，这里结束可以标识为status=5
                    ...
                    if data['history'] == []:
                        ...
                        if data['user'] == record_data['user_question']:
                            ...
                            #system_prompt = prompt_role1 + category + prompt_role2 + prompt_probe
                            response = '4 四'
                            while '4' in response or '四' in response:
                            ...
                            result = {'user_id': index, 'response': response, 'history': history, 'status': '4', 'category': category}
                            with open('./user_records_1/'+str(index)+'.json', 'r', encoding='utf-8') as f:
                                record_data = json.load(f)
                            record_data[model_name] = {'history': history, 'status': '4', 'category': category, 'symptom': symptom, 'starttime': data['start_time']}
                            with open('./user_records_1/'+str(index)+'.json', 'w', encoding='utf-8') as f:
                                json.dump(record_data, f, indent=4, ensure_ascii=False)
                            return result
                    ...
                    # 非第一次请求
                        ...
                        if ('诊断：' in response or '建议：' in response) and ('?' not in response and '？' not in response):
                            ...
                            if '否' in if_ask_response:
                                ...
                                if '初步诊断已明确，请线下就医' in if_ask_more:
                                    # 专家组讨论
                                    ...
                                    result = {'user_id': index, 'response': response.choices[0].message.content, 'history': history_result, 'status': '3', 'category': data['category']}
                                    with open('./user_records_1/'+str(index)+'.json', 'r', encoding='utf-8') as f:
                                        record_data = json.load(f)
                                    record_data[model_name]['history'] = history
                                    ...
                                    result = {'user_id': index, 'response': if_ask_more, 'history': history_new, 'status': '4', 'category': data['category']}
                                    with open('./user_records_1/'+str(index)+'.json', 'r', encoding='utf-8') as f:
                                        record_data = json.load(f)
