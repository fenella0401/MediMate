...
# 加载pulse模型
model_name = "/devdata/fenella/project/llm/PULSE-7bv5"
                ...
                if data['status'] != '4':
                    ...
                    if data['history'] == []:
                        ...
                        if '是' in system_response.choices[0].message.content:       
                            ...
                            history = [{'role': 'user', 'content': "假设你是一位经验丰富并且非常谨慎的的医生，会通过和患者进行多次的问答来明确自己的猜测，并且每次只能提一个问题，最终只会推荐相应的检验、检查、就诊科室以及疑似的诊断，请回复患者的提问：\n" + data['user']}]
                            response = pulse(history)
                            ...
                            if '否' not in if_ask_response or '?' in response or '？' in response:
...
def api_gettask():
    ...
    # # 判断如果队列和res同时为空，return status=0
    # if request_queue.empty() and res == {}: 
    #     result = {'status': '0'}
