...
# 加载DISC-MedLLM模型
tokenizer = AutoTokenizer.from_pretrained("/devdata/fenella/project/llm/DISC-MedLLM-model", use_fast=False, trust_remote_code=True)
model = AutoModelForCausalLM.from_pretrained("/devdata/fenella/project/llm/DISC-MedLLM-model", torch_dtype=torch.float16, trust_remote_code=True)
model = model.quantize(8).cuda()
model.generation_config = GenerationConfig.from_pretrained("/devdata/fenella/project/llm/DISC-MedLLM-model")
...
prompt_anaylsis3 = '"""。回答内容包括：1.病史梳理，2.病情分析（说明分析原因），3.诊疗建议（推荐就医科室、就医紧急程度、推荐检查、初步治疗方法），4.生活习惯建议。'
                ...
                if data['status'] != '4':
                    ...
                    if data['history'] == []:
                        ...
                        if '是' in system_response.choices[0].message.content:       
                            ...
                            response = '您好，感谢您的耐心等候，我是智能健康咨询机器人医生。' + response.choices[0].message.content
                    ...
                    # 非第一次请求
                        ...
                        if '诊断：' in response or '建议：' in response:
                            if_ask_prompt = [
                                    {
                                        "role": "user",
                            ...
                            if '否' in if_ask_response:
                                ...
                                response_init = client.chat.completions.create(model="glm-4",messages=[{'role':'user', 'content': prompt_role1 + data['category'] + prompt_role2 + prompt_anaylsis1+prompt_history+prompt_anaylsis2+history[1]['content']+prompt_anaylsis3}])
                                response_init = response_init.choices[0].message.content

                                # 专家组讨论
                                # 你是一个经验丰富的临床医生。以下是一份诊断记录，你需要找出其中病情分析、诊疗建议或生活习惯建议中的错误，并说明。
                                messages = [
                                        {
                                            "role": "user",
                                            "content": "以下是一份诊断记录：\n\"\"\"\n" + response_init + "\n\"\"\"\n\n你是一位经验丰富的临床医生。请你找出该诊断记录中病情分析、诊疗建议或生活习惯建议中的错误，并说明。"
                                        },
                                    ]
                                response = client.chat.completions.create(model="glm-4",messages=messages)

                                messages.append({"role": "assistant", "content": response.choices[0].message.content})
                                messages.append({"role": "user", "content": "你是另一位经验丰富的临床医生。基于诊断记录和上一位医生的想法，请问你还有什么不同的或补充的意见吗？"})
                                response = client.chat.completions.create(model="glm-4",messages=messages)

                                messages.append({"role": "assistant", "content": response.choices[0].message.content})
                                messages.append({"role": "user", "content": "您是一位专家大夫。基于诊断记录和前面医生的讨论，请您做出专业的判断或补充，并为患者重新生成问诊回复，内容包括：1.病史梳理，2.病情分析（说明分析原因），3.诊疗建议（推荐就医科室、就医紧急程度、推荐检查、初步治疗方法），4.生活习惯建议。并以以下内容为开头：“您好，感谢您的耐心等候。根据您的情况，智能健康咨询机器人医生团队进行了充分讨论，最终诊断与建议如下：\n”"})
                                response = client.chat.completions.create(model="glm-4",messages=messages)

                                history_result = [{'role':'user', 'content': prompt_role1 + data['category'] + prompt_role2 + prompt_anaylsis1+prompt_history+prompt_anaylsis2+history[1]['content']+prompt_anaylsis3}, {'role':'assistant', 'content': response_init}]
                                history_result.extend(messages)
                                history_result.append({"role": "assistant", "content": response.choices[0].message.content})
                ...
                # 体验第二个系统 status=4 前一系统已体验结束，这里结束可以标识为status=5
                    ...
                    if data['history'] == []:
                        ...
                        if data['user'] == record_data['user_question']:
                            response = model.chat(tokenizer, [{'content': data['user'], 'role': 'user'}])
                            history = [{'role': 'user', 'content': data['user']}, {'role': 'assistant', 'content': response}]
                            ...
                            if_ask_response = client.chat.completions.create(model="glm-4",messages=if_ask_prompt)
                            if_ask_response = if_ask_response.choices[0].message.content
                            print(if_ask_response)

                            if '否' not in if_ask_response:
                                result = {'user_id': index, 'response': response, 'history': history, 'status': '4', 'category': ''}
                                with open('./user_records_1/'+str(index)+'.json', 'r', encoding='utf-8') as f:
                                    record_data = json.load(f)
                                record_data[model_name] = {'history': history, 'status': '4', 'starttime': data['start_time']}
                                with open('./user_records_1/'+str(index)+'.json', 'w', encoding='utf-8') as f:
                                    json.dump(record_data, f, indent=4, ensure_ascii=False)
                                return result
                            else:
                                result = {'user_id': index, 'response': response, 'history': history, 'status': '3', 'category': ''}
                                with open('./user_records_1/'+str(index)+'.json', 'r', encoding='utf-8') as f:
                                    record_data = json.load(f)
                                record_data[model_name] = {'history': history, 'status': '3', 'starttime': data['start_time'], 'endtime': time.time()}
                                with open('./user_records_1/'+str(index)+'.json', 'w', encoding='utf-8') as f:
                                    json.dump(record_data, f, indent=4, ensure_ascii=False)
                                return result
                        else:
                    ...
                    # 非第一次请求
                        ...
                        if '否' not in if_ask_response:
                            ...
                            with open('./user_records_1/'+str(index)+'.json', 'r', encoding='utf-8') as f:
                                record_data = json.load(f)
                            record_data[model_name]['history'] = history
                            record_data[model_name]['status'] = '4'
                            with open('./user_records_1/'+str(index)+'.json', 'w', encoding='utf-8') as f:
                                json.dump(record_data, f, indent=4, ensure_ascii=False)
                            return result
                        else:
...
def api_submit():
    ...
    with open('./user_records_test/'+str(data['user_id'])+'.json', 'r', encoding='utf-8') as f:
        record_data = json.load(f)
    record_data['submit'] = data['submit']
    with open('./user_records_test/'+str(data['user_id'])+'.json', 'w', encoding='utf-8') as f:
        json.dump(record_data, f, indent=4, ensure_ascii=False)
